...
// Это обеспечивает нахождение функций расширений OpenGL по их имени
  glfunc func_pointer=NULL;
  func_pointer=glXGetProcAddress((GLubyte*)func_name);
  if(func_pointer==NULL)logout<<OSAPP::msg[OSAPP::err_notsupported_ext]<<func_name<<NS;
  return func_pointer;
}
*/

//PFNGLXSWAPINTERVALSGIPROC        glXSwapIntervalSGI;
//REG_GLEXT_FUNC(PFNGLXSWAPINTERVALSGIPROC,glXSwapIntervalSGI);

GLAPP::GLAPP():GRAPHCONTEXT(),xdisplay(NULL),xcontext(NULL),xcursor(0),xwindow(0),
	       mode(false),omode(false),cursor(true),ocursor(true),
	       osync(false),sync(false),ogamma(),gamma(vec3::one){
...
bool GLAPP::open(){
  ...
  if(buffering==2){
    xDoubleBuffered=true;
    va.push_back(GLX_DOUBLEBUFFER);
  }else{
    xDoubleBuffered=false;
  }
  ...
  xwindow=XCreateWindow(xdisplay,
			...
			(unsigned int)size.x, (unsigned int)size.y, // Width and height of window
			0,        // Border width
			xvisualInfo->depth,
			InputOutput,
			xvisualInfo->visual,
...
void GLAPP::update(){ // Check Program Change Window Geometry
  ...
  if(osync!=sync){ // Vertical sync
    ...
    //  glXSwapIntervalSGI((int)sync); Временно отключаю, это вызывает проблемы в моей системе!!!
    //}
    osync=sync;
  }
...
bool GLAPP::run(){ // X message translation
  ...
  while(key.array.size()>0)key.array.pop_front(); // Erase all key events
...
void GLAPP::ubind(){
  if(xdisplay && xwindow){
    if(xDoubleBuffered)glXSwapBuffers(xdisplay,xwindow);else glFlush();
